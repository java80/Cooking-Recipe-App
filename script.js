...
async function getRandomDish(event) {
  ...
  try {
    let randomResponse = await axios.get('https://www.themealdb.com/api/json/v1/1/random.php')
...
function showListofMeals(meals) {
  ...
  if (meals) {
    ...
    console.log("no item")
    messagetouser.innerText = "no dish found with the keyword " + searchTerm.value;
  }
    ...
    for (let i = 1; i < 30; i++){
     ...
     // console.log(element['strIngredient' + i])
      if (element['strIngredient' + i]) {
        ...
        row.innerText = element["strMeasure" + i] + " - " + element["strIngredient" + i]
